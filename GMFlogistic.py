...
if __name__ == '__main__':
    for topK in range(10, 11):
        ...
        print(
            "GMF-logistic (%s) Settings: num_factors=%d, batch_size=%d, learning_rate=%.1e, num_neg=%d, weight_neg=%.2f, regs=%s, epochs=%d, verbose=%d, topK=%d"
            % (learner, num_factors, batch_size, learning_rate, num_negatives, weight_negatives, regs, epochs, verbose, topK))
        ...
        for epoch in range(epochs):
            ...
            if epoch % verbose == 0:
                ...
                if hr > best_hr:
                    best_hr = hr
                    if hr > 0.4:
                        model.save_weights('pretrain/%s/top%d_%s_GMF_%d_neg_%d_hr_%.4f_ndcg_%.4f.h5' % (
                            learner.lower(), topK, dataset_name, num_factors, num_negatives, hr, ndcg), overwrite=True)
                if ndcg > best_ndcg:
                    best_ndcg = ndcg
