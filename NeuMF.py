...
def load_pretrain_model(model, gmf_model, mlp_model, num_layers):
    ...
    model.get_layer('prediction').set_weights([0.7 * new_weights, 0.3 * new_b])
    return model
...
if __name__ == '__main__':
    ...
    mf_pretrain = '/mnt/Data/Projects/neural_collaborative_filtering/pretrain/adam/ml-1m_GMF_32_neg_1_hr_0.6930_ndcg_0.4079.h5'
    mlp_pretrain = '/mnt/Data/Projects/neural_collaborative_filtering/pretrain/adam/ml-1m_MLP_32_neg_1_hr_0.6659_ndcg_0.3875.h5'
    ...
    print(
        "NeuMF(%s) Dropout %s: mf_dim=%d, layers=%s, regs=%s, reg_mf=%.1e, num_negatives=%d, weight_negatives=%.2f, learning_rate=%.1e, num_epochs=%d, batch_size=%d, verbose=%d topK=%d"
        % (learner, enable_dropout, mf_dim, layers, reg_layers, reg_mf, num_negatives, weight_negatives, learning_rate,
           num_epochs, batch_size, verbose, topK))
    ...
    if hr > 0.6:
        model.save_weights(
            'pretrain/%s_NeuMF_%d_neg_%d_hr_%.4f_ndcg_%.4f.h5' % (dataset_name, layers[-1], num_negatives, hr, ndcg),
            overwrite=True)
    ...
    for epoch in range(num_epochs):
        ...
        if epoch % verbose == 0:
            ...
            if hr > best_hr:
                best_hr = hr
                if hr > 0.6:
                    model.save_weights('pretrain/raw_%s_NeuMF_%d_neg_%d_hr_%.4f_ndcg_%.4f.h5' % (
                        dataset_name, layers[-1], num_negatives, hr, ndcg), overwrite=True)
            if ndcg > best_ndcg:
                best_ndcg = ndcg
